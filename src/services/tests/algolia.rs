...
#[test]
fn request_body_preserves_gitmoji_search_contract() -> Result<()> {
    let client = AlgoliaSearch::with_base_url(config(), Url::parse("http://localhost/")?)?;
    let body: Value = serde_json::from_str(&client.request_body("bug")?)?;

    assert_eq!(
        body,
        json!({
            "query": "bug",
            "attributesToRetrieve": ["emoji", "entity", "code", "description", "name", "semver"],
            "page": 0,
            "hitsPerPage": 100
        })
    );
...
fn query_retries_retryable_failures_in_host_order() -> Result<()> {
    ...
    let hits = client.query_with("bug", |endpoint, _, _| {
    ...
    assert_eq!(
        endpoints,
        vec![
            "http://first.test/1/indexes/gitmoji/query",
            "http://second.test/1/indexes/gitmoji/query",
            "http://third.test/1/indexes/gitmoji/query",
        ]
    );
    Ok(())
}
...
fn query_reports_all_retryable_host_failures() -> Result<()> {
    ...
    assert!(message.contains("first.test/1/indexes/gitmoji/query failed"));
    assert!(message.contains("second.test/1/indexes/gitmoji/query failed"));
    Ok(())
}
...
fn query_uses_one_shared_timeout_budget() -> Result<()> {
    ...
    let _ = client.query_with("bug", |_, _, remaining| {
...
fn query_includes_4xx_response_body() -> Result<()> {
    ...
    let error = client.query("bug").expect_err("4xx responses must fail");
    server
        .join()
...
fn query_rejects_malformed_successful_response() -> Result<()> {
    let (url, server) = serve_once(200, r#"{"hits":"not an array"}"#)?;
    let client = AlgoliaSearch::with_read_hosts_and_agent(config(), vec![url], no_proxy_agent())?;
