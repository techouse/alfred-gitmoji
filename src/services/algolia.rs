...
/// Configuration required to query the Gitmoji Algolia index.
...
/// Synchronous client for the Gitmoji Algolia index.
#[derive(Clone)]
...
struct SearchRequest<'a> {
    query: &'a str,
    attributes_to_retrieve: [&'static str; 6],
    page: u8,
    hits_per_page: u8,
}
...
impl AlgoliaSearch {
    ...
    /// Searches the Gitmoji index and returns at most 100 ranked hits.
    ...
    fn request_body(&self, query: &str) -> Result<String> {
        Ok(serde_json::to_string(&SearchRequest {
            query,
            attributes_to_retrieve: ["emoji", "entity", "code", "description", "name", "semver"],
            page: 0,
            hits_per_page: 100,
        })?)
    }
